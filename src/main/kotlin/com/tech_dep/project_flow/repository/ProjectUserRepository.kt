...
interface ProjectUserRepository : JpaRepository<ProjectUser, Long> {
    ...
    @Query("SELECT pu FROM ProjectUser pu JOIN pu.user u WHERE pu.project.uuid = :projectId")
    fun findAllByProjectUuid(projectId: UUID, pageable: Pageable): Page<ProjectUser>
